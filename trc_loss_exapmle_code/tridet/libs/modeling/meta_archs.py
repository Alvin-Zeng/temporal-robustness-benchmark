...
class TriDet(nn.Module):
    ...
    def __init__(
        ...
        assert self.loss_type in ['jsd', 'kld', 'mse', 'none']
        self.jsd_loss_weight = train_cfg['jsd_loss_weight']
        self.kld_loss_weight = train_cfg['kld_loss_weight']
        self.mse_loss_weight = train_cfg['mse_loss_weight']
        self.sample_way = train_cfg['sample_way']
    ...
    def forward(self, video_list):
        if self.training and self.loss_type != 'none':
            
            video_input = []
            # change tuple to list
            for i in range(len(video_list)):
                video_input.append(video_list[i][0])
                video_input.append(video_list[i][1])
            aug_video_list = video_input[1::2]
            video_list = video_input[0::2]
        ...
        if self.training and self.loss_type != 'none':
            aug_batched_inputs, aug_batched_masks = self.preprocessing(aug_video_list)
            aug_feats, aug_masks = self.backbone(aug_batched_inputs, aug_batched_masks)
            aug_fpn_feats, aug_fpn_masks = self.neck(aug_feats, aug_masks)
            aug_out_cls_logits = self.cls_head(aug_fpn_feats, aug_fpn_masks)
            if self.use_trident_head:
                aug_out_lb_logits = self.start_head(aug_fpn_feats, aug_fpn_masks)
                aug_out_rb_logits = self.end_head(aug_fpn_feats, aug_fpn_masks)
            else:
                aug_out_lb_logits = None
                aug_out_rb_logits = None
            aug_out_offsets = self.reg_head(aug_fpn_feats, aug_fpn_masks)

            aug_out_cls_logits = [x.permute(0, 2, 1) for x in aug_out_cls_logits]
            aug_out_offsets = [x.permute(0, 2, 1) for x in aug_out_offsets]
            aug_fpn_masks = [x.squeeze(1) for x in aug_fpn_masks]
        ...
        if self.training:
            ...
            if self.sample_way == 'whole':
                gt_cls_labels, gt_offsets, whole_gt_cls_labels = self.label_points(
            ...
            if self.loss_type != 'none':
                if self.sample_way != 'whole':
                    whole_gt_cls_labels = None
                losses, iou_rate = self.losses(
                    fpn_masks,
                ...
                _, aug_iou_rate = self.losses(
                    aug_fpn_masks, # must be the same
                    aug_out_cls_logits, aug_out_offsets,
                    gt_cls_labels, gt_offsets,
                    aug_out_lb_logits, aug_out_rb_logits,
                    whole_gt_cls_labels
                )
            else:
                losses = self.losses(
            ...
            if self.loss_type != 'none':
                if self.loss_type == 'jsd':
                    JSD_losses = self.JS_divergence_consistency_loss(
                        iou_rate, aug_iou_rate
                    )
                    JSD_losses = JSD_losses * self.jsd_loss_weight
                    losses['JSD_loss'] = JSD_losses
                    losses['final_loss'] += JSD_losses
                elif self.loss_type == 'kld':
                    KLD_losses = self.KL_divergence_consistency_loss(
                        iou_rate, aug_iou_rate
                    )
                    KLD_losses = KLD_losses * self.kld_loss_weight
                    losses['KLD_loss'] = KLD_losses
                    losses['final_loss'] += KLD_losses    
                elif self.loss_type == 'mse': 
                    MSE_losses = self.MSE_loss(
                        iou_rate, aug_iou_rate
                    )
                    MSE_losses = MSE_losses * self.mse_loss_weight
                    losses['MSE_losses'] = MSE_losses
                    losses['final_loss'] += MSE_losses  
            return losses
    ...
    def label_points(self, points, gt_segments, gt_labels):
        ...
        gt_cls, gt_offset, whole_cls = [], [], []

        # loop over each video sample
        for gt_segment, gt_label in zip(gt_segments, gt_labels):
            if self.sample_way == 'whole':
                cls_targets, reg_targets, whole_cls_targets = self.label_points_single_video(
                concat_points, gt_segment, gt_label
            )
                gt_cls.append(cls_targets)
                gt_offset.append(reg_targets)    
                whole_cls.append(whole_cls_targets)
        ...
        if self.sample_way == 'whole':
            return gt_cls, gt_offset, whole_cls
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        if self.sample_way == 'whole':
            segment_left = concat_points[:, 0, None] - gt_segs[:, :, 0]
            segment_right = gt_segs[:, :, 1] - concat_points[:, 0, None]
            whole_seg = torch.stack((segment_left, segment_right), -1)
            whole_gt_seg_mask = whole_seg.min(-1)[0] > 0            
        ...
        if self.sample_way == 'whole':
            whole_lens.masked_fill_(whole_gt_seg_mask == 0, float('inf'))
            whole_lens.masked_fill_(inside_regress_range == 0, float('inf'))
            whole_min_len, whole_min_len_inds = whole_lens.min(dim=1)
            whole_min_len_mask = torch.logical_and(
                (lens <= (whole_min_len[:, None] + 1e-3)), (whole_lens < float('inf'))
            ).to(reg_targets.dtype)
        ...
        if self.sample_way == 'whole':
            whole_cls_targets = whole_min_len_mask @ gt_label_one_hot
            whole_cls_targets.clamp_(min=0.0, max=1.0)

        if self.sample_way == 'whole':
            return cls_targets, reg_targets, whole_cls_targets
        else:
            return cls_targets, reg_targets
    ...
    def losses(
        ...
        if self.sample_way == 'whole':
            whole_gt_cls = torch.stack(whole_gt_cls_labels)
            whole_pos_mask = torch.logical_and((whole_gt_cls.sum(-1) > 0), valid_mask)            

        decoded_offsets = self.decode_offset(out_offsets, out_start_logits, out_end_logits)  # bz, stack_T, num_class, 2

        if self.sample_way == 'whole':
            whole_decoded_offsets = decoded_offsets[whole_pos_mask]
        if self.sample_way == 'both':
            both_decoded_offsets = decoded_offsets
        ...
        if self.use_trident_head:
            ...
            if self.sample_way == 'whole':
                whole_pred_offsets = whole_decoded_offsets[whole_gt_cls[whole_pos_mask].bool()] # select the highest score class
                whole_vid = torch.where(whole_gt_cls[whole_pos_mask])[0]
                whole_gt_offsets = torch.stack(gt_offsets)[whole_pos_mask][whole_vid]
                
            elif self.sample_way == 'both':
                both_pred_offsets = both_decoded_offsets[:,:,0,:].reshape(-1,2) # select the highest score class
                both_gt_offsets = torch.stack(gt_offsets).reshape(-1,2)

            pred_offsets = decoded_offsets[gt_cls[pos_mask].bool()] # select the highest score class
        ...
        # focal loss
        if self.loss_type != 'none':
            if self.sample_way == 'center_sample':
                TRC_iou = ctr_giou_loss_1d(
                    pred_offsets,
                    gt_offsets,
                    reduction='none'
                )
            elif self.sample_way == 'whole':
                TRC_iou = ctr_giou_loss_1d(
                    whole_pred_offsets,
                    whole_gt_offsets,
                    reduction='none'
                ...
                # split negtive ground-truth
                gt_neg = torch.logical_and((both_gt_offsets[:,0]>=0.0), (both_gt_offsets[:,1]>=0.0)).unsqueeze(1).expand_as(both_gt_offsets)
                pred_neg = torch.logical_and((both_pred_offsets[:,0]>=0.0), (both_pred_offsets[:,1]>=0.0)).unsqueeze(1).expand_as(both_pred_offsets)
                neg_pos = torch.logical_and(gt_neg, pred_neg)
                both_gt_offsets = both_gt_offsets[neg_pos].reshape(-1,2)
                both_pred_offsets = both_pred_offsets[neg_pos].reshape(-1,2)
                TRC_iou = ctr_giou_loss_1d(
                    both_pred_offsets,
                    both_gt_offsets,
                    reduction='none'
        ...
        if self.loss_type != 'none':
            return {'cls_loss': cls_loss,
                    'reg_loss': reg_loss,
                    'final_loss': final_loss}, TRC_iou
        else:
            return {'cls_loss': cls_loss,
                    'reg_loss': reg_loss,
                    'final_loss': final_loss}            

    def JS_divergence_consistency_loss(
        self,
        ori_iou_rate,
        aug_iou_rate
    ):
        """"
        input: two tiou from clean data and augmentation data
        """
        M = (ori_iou_rate + aug_iou_rate) / 2
        ori_KLDloss = (M * (M.log() - ori_iou_rate.log())).sum()
        aug_KLDloss = (M * (M.log() - aug_iou_rate.log())).sum()
        JSD_loss = (ori_KLDloss + aug_KLDloss) / 2
        return JSD_loss
        
    def KL_divergence_consistency_loss(
        self,
        ori_iou_rate,
        aug_iou_rate
    ):
        ...
        input: two tiou from clean data and augmentation data
        """
        return abs((ori_iou_rate * (ori_iou_rate.log() - aug_iou_rate.log())).sum())
    ...
    def MSE_loss(
        ...
        input: two tiou from clean data and augmentation data
        """
        return ((ori_iou_rate - aug_iou_rate) ** 2).mean()
