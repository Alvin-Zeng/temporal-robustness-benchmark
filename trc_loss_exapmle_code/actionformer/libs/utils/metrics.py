...
def visualization(vid, gt_boxes, pred_boxes, gt_label, pred_label, pred_score, saved_plt_root):
    x_gt=[0]
    top_k=50
    if not os.path.exists(saved_plt_root):
        os.makedirs(saved_plt_root)
    pred_duration=np.clip(pred_boxes[:,1]-pred_boxes[:,0],a_min=0,a_max=1e6)
    gt_duration=np.clip(gt_boxes[:,1]-gt_boxes[:,0],a_min=0,a_max=1e6)
    x_pred=[i for i in range(len(x_gt), min((len(x_gt) + top_k),len(pred_duration)+1))]
    sort_idx=pred_score.argsort()[::-1]
    sort_idx=sort_idx[:top_k]
    pred_score=pred_score[sort_idx]
    pred_label=pred_label[sort_idx]
    pred_boxes=pred_boxes[sort_idx]
    pred_duration=pred_duration[sort_idx]
    
    label = []
    for i in range(pred_score.size):
        sco = pred_score.tolist()[i]
        cls = pred_label.tolist()[i]
        label.append(f'{sco:.2f} & {cls}')

    bar_height = 0.8
    plt.figure(figsize=(8, 75))
    plt.barh(x_pred, np.array(pred_duration), height=bar_height, left=pred_boxes[:,0], tick_label=label)
    plt.barh(x_gt, gt_duration, height=bar_height, left=gt_boxes[:,0])
    plt.title(f"video_id:{vid} \nclass:{gt_label}")

    plt.xlabel("time")
    plt.ylabel("score & class")
    plt.gca().invert_yaxis()
    print(f'Saved in {os.path.join(saved_plt_root, vid)}')
    plt.savefig(f"{os.path.join(saved_plt_root, vid)}.png")

def preds_plot(ground_truth, preds, saved_plt_root):
    for index_video in [4,7,11]:
        gt_boxes = []
        gt_label = []
        pred_boxes = []
        pred_label = []
        ...
        for _,row in ground_truth.iterrows():
            if int(row['video-id'][14:18]) == index_video:
                gt_boxes.append([row['t-start'], row['t-end']])
                gt_label.append(row['label'])
        ...
        for _,row in preds.iterrows():
            if int(row['video-id'][14:18]) == index_video:
                pred_boxes.append([row['t-start'], row['t-end']])
                pred_label.append(row['label'])
                pred_score.append(row['score'])

        pred_boxes = np.array(pred_boxes)
        pred_label = np.array(pred_label)
        pred_score = np.array(pred_score)
        vid = 'video_test_'+'%07d' % index_video

        visualization(vid, gt_boxes, pred_boxes, gt_label, pred_label, pred_score, saved_plt_root)
...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        # preds_plot(ground_truth, preds, './save_img')
    ...
    def evaluate(self, preds, verbose=True):
        ...
        # print results
        if verbose:
            if self.extractor == 'i3d':
                with open('./result_i3d.txt','a') as f:
                    # print the results
                    ...
                    for tiou, tiou_mAP, tiou_mRecall in zip(self.tiou_thresholds, mAP, mRecall):
                        if tiou == 0.5:
                            print(f'mAP(tIoU=0.5) is: {tiou_mAP}', file=f)
                        block += '\n|tIoU = {:.2f}: '.format(tiou)
                ...
                with open('./result_mae.txt','a') as f:
                    ...
                    print('Average mAP: {:>4.2f} (%)'.format(average_mAP*100))
        # return the results
        return mAP[2], average_mAP, mRecall
