...
class TemporalMaxerArchitecture(nn.Module):
    def __init__(self,
        ...
        self.use_jsdloss = train_cfg['use_jsdloss']
        self.jsdloss_weight = train_cfg['jsdloss_weight']
    ...
    def forward(self, video_list):
        if self.training and self.use_jsdloss:
            video_input = []
        ...
        if self.training:
            ...
            if self.use_jsdloss:
                aug_batch_fpn_masks = torch.cat(aug_fpn_masks, dim=1)
                aug_out_cls_logits = torch.cat(aug_out_cls_logits, dim=1)
                aug_out_offsets = torch.cat(aug_out_offsets, dim=1)
                aug_targets = self.get_target(aug_out_cls_logits, points, aug_out_offsets,
                                        gt_segments, gt_labels, aug_batch_fpn_masks,
                                        batch_size) 
                aug_reg_targets = aug_targets['reg_targets']
                aug_cls_targets = aug_targets['cls_targets']
                aug_weights = aug_targets['weights']

                losses, iou_rate = self.losses(fpn_masks, out_cls_logits,
                                    out_offsets, cls_targets,
                                    reg_targets, weights)
                # compute the loss and return

                _, aug_iou_rate = self.losses(aug_fpn_masks, aug_out_cls_logits,
                                    aug_out_offsets, aug_cls_targets,
                                    aug_reg_targets, aug_weights)
                
                JSD_losses = self.JS_divergence_consistency_loss(
                    iou_rate, aug_iou_rate
                )
                JSD_losses = JSD_losses * self.jsdloss_weight
    ...
    def JS_divergence_consistency_loss(
        ...
        """"
        input: two tiou from original data and augmentation data
        """
    ...
    def losses(
        ...
        if self.use_jsdloss:
            return {'cls_loss': cls_loss,
                    ...
                    'num_pos': pos_weights.sum()}, iou_rate
        else:
            return {'cls_loss': cls_loss,
                    'reg_loss': reg_loss,
                    'final_loss': final_loss,
