...
def extract_feature(args):
    ...
    # get video path
    vid_list = [i for i in os.listdir(args.data_path) if i.endswith('mp4')]
    # vid_list.sort(reverse=True)
    ...
    # per_shard_num = num_videos//args.num_shards
    # if args.shard_id < args.num_shards - 1:
    #     vid_list = vid_list[per_shard_num*args.shard_id : per_shard_num*(args.shard_id+1)]
    # else:
    #     vid_list = vid_list[per_shard_num*args.shard_id : ]
    ...
    for idx, vid_name in enumerate(vid_list):
        ...
        try:
            with open('./processed.json','r') as f:
                processed_vid = json.load(f)
        except:
            continue


        if os.path.exists(url):
            processed_vid[vid_name] = 1
            continue
        try:
            if processed_vid[vid_name] == 1:
                continue
            else:
                pass
        except:
        ...
        with open('./processed.json','w') as f:
            json.dump(processed_vid, f, indent=4)

        print(f'video name is: {vid_name}')
        ...
        for start_idx in start_idx_range(len(vr)):
            if start_idx % 80 == 0:
                print(f'{vid_name}: {start_idx} / {len(vr)}')

            data = vr.get_batch(np.arange(start_idx, start_idx + 16))
        ...
        print(f'[{idx} / {len(vid_list)}]: save feature on {url}')
